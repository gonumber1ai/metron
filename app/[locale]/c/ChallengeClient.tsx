...
import { Testimonials } from "@/components/c/Testimonials";
...
export function ChallengeClient({ locale, geoCountry }: { locale: string; geoCountry?: string | null }) {
  ...
  const Go = ({ where, className = "", compact = false }: { where: string; className?: string; compact?: boolean }) => <Link href={`/${locale}/offer?go=1`} onClick={() => track("start_cta", `c_${where}`, locale)} className={`btn-lime c-cta ${className}`}><span className="c-cta-label">{compact ? l.ctaShort : l.cta}</span><span className="c-cta-price">{price}</span></Link>;
  ...
  const Rail = ({ slot, head }: { slot: "journey" | "mindset" | "partner" | "proof" | "progression"; head: string }) => <Testimonials items={getTestimonials(locale, slot)} heading={head} planLabels={{ "10": l.plan10, "30": l.plan30 }} prevLabel={l.railPrev} nextLabel={l.railNext} />;
  ...
  return <><style>{`body{background:#030303;color:#fff}body::before{display:none}`}</style><MetaPixel event="ViewContent" /><div className="c-page"><header className="c-bar"><div className="c-bar-inner"><span className="c-brand"><Logo size="sm" /><small>{l.tagline}</small></span>{passed ? <><div className="c-bar-timer"><Countdown label={l.timer} hrs={units[0]} mins={units[1]} secs={units[2]} expiredLabel={l.expired} compact /></div><Go where="bar" compact /></> : <Link href={`/${locale}/login`} className="c-bar-login">{l.login}</Link>}</div></header><main className="c-shell">
    ...
    </div></main>{passed && <div className="c-mobile-buy"><Countdown label={l.timer} hrs={units[0]} mins={units[1]} secs={units[2]} expiredLabel={l.expired} compact /><Go where="mobile" compact /></div>}</div></>;
}
