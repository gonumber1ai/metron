...
export function ChallengeClient({ locale, geoCountry }: { locale: string; geoCountry?: string | null }) {
  ...
  const complete = useCallback(() => { setPassed(true); track("quiz_complete", "c", locale); window.setTimeout(() => reveal.current?.scrollIntoView({ behavior: "smooth", block: "start" }), 80); }, [locale]);
  ...
  const Go = ({ where, className = "" }: { where: string; className?: string }) => <Link href={`/${locale}/offer?go=1`} onClick={() => track("start_cta", `c_${where}`, locale)} className={`btn-lime c-cta ${className}`}>{l.cta}</Link>;
  ...
  return <><style>{`body{background:#030303;color:#fff}body::before{display:none}`}</style><MetaPixel event="ViewContent" /><div className="c-page"><header className="c-bar"><div className="c-bar-inner"><span className="c-brand"><Logo size="sm" /><small>{l.tagline}</small></span><div className="c-bar-timer"><Countdown label={l.timer} hrs={units[0]} mins={units[1]} secs={units[2]} expiredLabel={l.expired} compact /></div><Go where="bar" /></div></header><main className="c-shell">
    <section className="c-hero c-photo"><Image src="/c/hero.jpg" fill priority sizes="(max-width: 768px) 100vw, 1100px" alt="" className="c-photo-image" /><div className="c-photo-shade" /><div className="c-hero-copy"><h1 className="c-heading">{l.heroA}<br /><span>{l.heroB}</span><br />{l.heroC}</h1><i className="c-rule" /><p className="c-lede">{l.heroSub}</p><div className="c-benefits">{benefits.map(([icon, title, sub]) => <div key={title}><b><Icon name={icon as IconName} /></b><strong>{title}</strong><small>{sub}</small></div>)}</div><p className="c-script">{l.script}</p></div></section>
    <section className="c-quiz"><Quiz3 quals={quiz} ofLabel={l.question} privateNote={l.private} onDone={complete} /></section>
    <div ref={reveal} className={passed ? "c-reveal" : "hidden"} aria-hidden={!passed}>
      ...
      <section className="c-close"><div><Heading a={l.closeA} b={l.closeB} /><p className="c-lede">{(expired ? l.closeSubExpired : l.closeSub).replace("{price}", price)}</p><p className="c-close-note">{l.closeNote}</p></div><Countdown label={l.timer} hrs={units[0]} mins={units[1]} secs={units[2]} expiredLabel={l.expired} /><Go where="close" className="c-full" /><Trust /></section><footer className="c-footer">{marketing.disclaimer}</footer>
    </div></main><div className="c-mobile-buy"><Countdown label={l.timer} hrs={units[0]} mins={units[1]} secs={units[2]} expiredLabel={l.expired} compact /><Go where="mobile" /></div></div></>;
}
