...
export function ChallengeClient({ locale, geoCountry }: { locale: string; geoCountry?: string | null }) {
  ...
  useEffect(() => { try { const p = new URLSearchParams(window.location.search); const campaign = (p.get("c") ?? p.get("utm_campaign") ?? "").replace(/[^a-zA-Z0-9_-]/g, "").slice(0, 40); if (campaign) update((s) => s.campaign ? s : { ...s, campaign }, locale); } catch {} track("start_view", "c", locale); }, [locale]);
  ...
  const Go = ({ where, className = "", compact = false }: { where: string; className?: string; compact?: boolean }) => <Link href={`/${locale}/signup`} onClick={() => track("start_cta", `c_${where}`, locale)} className={`btn-lime c-cta ${className}`}><span className="c-cta-label">{compact ? l.ctaShort : l.cta}</span><span className="c-cta-price">{l.ctaSub}</span></Link>;
  ...
  return <><style>{`body{background:#030303;color:#fff}body::before{display:none}`}</style><MetaPixel event="ViewContent" /><div className="c-page"><header className="c-bar"><div className="c-bar-inner"><span className="c-brand"><Logo size="sm" /><small>{l.tagline}</small></span>{passed ? <><Go where="bar" compact /></> : <Link href={`/${locale}/login`} className="c-bar-login">{l.login}</Link>}</div></header><main className="c-shell">
    ...
    {!restored && <section className="c-quiz"><Quiz3 quals={quiz} ofLabel={l.question} kicker={l.askKicker} onDone={complete} /></section>}
      ...
      <VideoSpot cards={getVideos(locale)} locale={locale} heading={l.videoHead} playLabel={l.playLabel} planLabels={{ "10": l.plan10, "30": l.plan30 }} />
    <div ref={reveal} className={passed ? "c-reveal" : "hidden"} aria-hidden={!passed}>
      ...
      <section className="c-close"><div><Heading a={l.closeA} b={l.closeB} /><p className="c-lede">{l.closeSubFlat.replace("{price}", price)}</p><p className="c-close-note">{l.closeNote}</p></div><Go where="close" className="c-full" /><Trust /></section><footer className="c-footer">{marketing.disclaimer}</footer>
