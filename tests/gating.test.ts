...
test("p10: 18h after the previous checklist; Day 12 is 36h after Day 10; Day 11 never opens", () => {
  const p = { completed: { 1: iso(now - 20 * H) } };
  assert.equal(dayState(2, "p10", p, now).state, "open");
  const p2 = { completed: { 1: iso(now - 20 * H), 2: iso(now - 10 * H) } };
  ...
  const p10 = { completed: { 10: iso(now - 20 * H) } };
  assert.equal(dayState(12, "p10", p10, now).state, "locked", "20h is not 36h");
  assert.equal(dayState(12, "p10", { completed: { 10: iso(now - 37 * H) } }, now).state, "open");
  assert.equal(dayState(11, "p10", { completed: { 10: iso(now - 100 * H) } }, now).state, "locked");
  assert.equal(dayState(4, "p10", { completed: { 1: iso(now) } }, now).state, "locked", "cannot skip");
});

test("p30: Day 13 opens the moment he buys if Day 12 is done; 13+ locked on p10", () => {
  const p = { completed: { 12: iso(now - 1 * H) } };
  assert.equal(dayState(13, "p30", p, now).state, "open");
  assert.deepEqual(dayState(13, "p10", p, now), { state: "locked", reason: "pay" });
  assert.equal(dayState(13, "p30", { completed: {} }, now).state, "locked");
});

test("currentDay is the first unfinished day he may open, else the last one he finished", () => {
  assert.equal(currentDay("free", { completed: {} }, now), 1);
  assert.equal(currentDay("free", { completed: { 1: iso(now) } }, now), 1, "free stays on 1");
  assert.equal(currentDay("p10", { completed: { 1: iso(now - 20 * H) } }, now), 2);
  assert.equal(currentDay("p10", { completed: { 1: iso(now - 20 * H), 2: iso(now - 1 * H) } }, now), 2, "waiting shows the day just finished");
  assert.equal(currentDay("p10", { completed: { 10: iso(now - 40 * H), 1: iso(now - 200 * H) } }, now), 2, "skips nothing");
});
